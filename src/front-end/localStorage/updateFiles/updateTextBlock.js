 ...
 function handleOriginalSubComponent (textBlock, originalBlock, callback) {
	localStorage.readUser((err, user) => {
		if (err) {
			...
			if (textBlock.objectReference === originalBlock.objectReference) {
				callback(null, textBlock);
			} else {
				if (originalBlock.kind === "event") {
					let originalEvent = user.events.filter(event => event.id === originalBlock.objectReference)[0];
					originalEvent.references = originalEvent.references.filter(reference !== textBlock.id);
					...
					originalTask.references = originalTask.references.filter(reference !== textBlock.id);
  ...
  function handleNewSubComponent (textBlock, originalBlock, date, callback) {
	localStorage.readUser((err, user) => {
		if (err) {
			...
			if (textBlock.objectReference === originalBlock.objectReference && date) {
				if (textBlock.objectReference && originalBlock.kind === "event") {
					...
					event.references = event.references.filter(reference !== textBlock.id);
						...
						if (failedNewChange) {
							...
							localStorage.readUser((failedToRead, updatedUser) => {
								if (failedToRead) {
									...
									} else if ((updatedUser.textBlocks.filter(block => block.id === audioBlock.id))[0].parent !== textBlock.parent){
									...
									updatedUser.textBlock.push(newTextBlock);
									let newUser = {
										_id: "0000",
